...
-   **Nodes Data:**
    ...
    -   Columns `ProductServices`, `PointOfContact`, `HeadOfOrg`, `founding_date`, `revenue`, and `TradeDescription` have around 85% missing values.
...
-   **Links Data:**
    ...
    -   No missing values in the `source`, `target`, and `type` columns.

## Actions:

-   Drop Columns: If columns with around 85% missing values are not crucial for your analysis, consider dropping them.

-   Imputation: For columns like dob with about 14.7% missing values, consider imputing missing values with median or mean (if numerical) or a placeholder (if categorical).
:::
