...
### Clean Missing Values (Temp)

```{r}
# Inspect and clean nodes data
mc3_data$nodes <- mc3_data$nodes %>%
  filter_all(all_vars(!is.na(.)))  # Remove rows with any NA values

# Inspect and clean edges data
mc3_data$links <- mc3_data$links %>%
  filter_all(all_vars(!is.na(.)))  # Remove rows with any NA values

# Ensure all columns have proper data types (e.g., numeric for IDs)
mc3_data$nodes <- mc3_data$nodes %>%
  mutate(across(everything(), as.character))  # Convert all columns to character if necessary
...
id2 <- mc3_data$links %>%
  select(target) %>%
  rename (id = target)
mc3_data$nodes1 <- rbind(id1, id2) %>%
  distinct() %>%
  left_join(mc3_data$nodes,
            unmatched = "drop")
```
...
```{r}
mc3_graph <- tbl_graph(nodes = mc3_data$nodes1,
